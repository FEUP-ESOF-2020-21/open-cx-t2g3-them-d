...
class DatabaseService {
  ...
  Stream<List<SessionQuestion>> getQuizQuestions(String conferenceId){
    return conferencesCollection.doc(conferenceId).collection('sessions').snapshots().map(_conferenceQuizFromSnapshot);
  }
  ...
  Stream<List<Session>> getConferenceSessions(String conferenceId){
    return conferencesCollection.doc(conferenceId).collection('sessions').snapshots().map(_conferenceSessionsFromSnapshot);
  }
  ...
  Future<void> addSessionSuggestion(String conferenceId, String sessionId) async{
    return await sessionSuggestions.doc(uid).collection(conferenceId).doc(sessionId).set({});
  }
  ...
  Future<List<String>> getSuggestedSessions(String conferenceId) async {
    return await sessionSuggestions.doc(uid).collection(conferenceId).get().then
